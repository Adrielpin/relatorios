...
$selector->fields = array($sintax, 'Clicks', 'Impressions', 'Cost', 'AveragePosition', 'Conversions', 'ViewThroughConversions', $parcela_impressao, $parcela_impressao_orcamento, $parcela_impressao_rank, 'VideoViews');
...
foreach($arr as &$Row){
	...
	elseif($sintax == 'Month'){
		$date = $mes_extenso[date('m', strtotime($Row[0]))];
	} 
	else{
		$date = $Row[0];
	}
	...
	array_push($views, array($date, $Row[10]));
}
...
foreach($investimento as &$a){
	foreach($views as &$b){
		if($a[0] == $b[0]){
			array_push($cpm, array($a[0], round($a[1]/$b[1], 2)));
		}
	}	
}
...
foreach($views as &$a){
	foreach($impressoes as &$b){
		if($a[0] == $b[0]){
			array_push($vtr, array($a[0], round(($a[1]/$b[1])*100, 2)));
		} 
	}
}
...
foreach($posicao as &$Row){
	foreach ($impressoes as $k) {
		array_push($posicao_tabela, array($Row[0], (float)($k[1]/$Row[1]), number_format((float)($k[1]/$Row[1]),1,',','.')));
		$t_posicao += $Row[1];
		$count ++;
	}
}
...
$views_tabela = array(array('Data', 'Visualizações',array(type => 'string', role =>'annotation')));
$t_views = 0;
foreach($views as &$Row){
	array_push($views_tabela, array($Row[0], (int)$Row[1], number_format((int)$Row[1],0,',','.')));
	$t_views += $Row[1];
}

$cpm_tabela = array(array('Data', 'CPC', array(type => 'string', role =>'annotation')));
$t_cpm = $t_investimento/$t_views;
foreach($cpm as &$Row){
	array_push($cpm_tabela, array($Row[0], (float)$Row[1], 'R$ '.number_format((float)$Row[1],2,',','.')));
}

$vtr_tabela = array(array('Data', 'CTR %', array(type => 'string', role =>'annotation')));
$t_vtr = ($t_views/$t_impressoes)*100;
foreach($vtr as &$Row){
	array_push($vtr_tabela, array($Row[0], (float)$Row[1], number_format((float)$Row[1],2,',','.').' %'));
}
...
foreach($total_parcela as &$b){
	...
	foreach($investimento as &$a){
		if($a[0] == $b[0]){
			...
			array_push($investimento_x_parcela_date, array($a[0], (float)$a[1], 'R$ '.number_format($a[1],2,',','.'), (float)$v, 'R$ '.number_format((float)$v,2,',','.')));
...
var data = google.visualization.arrayToDataTable(".json_encode($views_tabela).");
new google.visualization.BarChart(document.getElementById('chart_view')).draw(data, VisualizacoesData);
document.getElementById('t_view').innerHTML='<h5> Total: ".number_format($t_views,0,',','.')."</h5>';

var data = google.visualization.arrayToDataTable(".json_encode($cpm_tabela).");
new google.visualization.BarChart(document.getElementById('chart_cpm')).draw(data, CPVmedioData);
document.getElementById('t_cpm').innerHTML='<h5> Média R$: ".number_format($t_cpm,2,',','.')."</h5>';

var data = google.visualization.arrayToDataTable(".json_encode($vtr_tabela).");
new google.visualization.BarChart(document.getElementById('chart_vtr')).draw(data, TaxaDevisualizacoesData);
document.getElementById('t_vtr').innerHTML='<h5> Média: ".number_format($t_vtr,2,',','.')."%</h5>';
