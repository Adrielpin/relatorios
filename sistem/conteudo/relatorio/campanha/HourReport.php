...
$selector->fields = array('HourOfDay', 'Clicks', 'Impressions', 'Cost', 'AveragePosition', 'Conversions', 'ViewThroughConversions', $parcela_impressao, $parcela_impressao_orcamento, $parcela_impressao_rank,'GmailSecondaryClicks','GmailSaves','GmailForwards');
...
foreach($gmailClique as &$Row){
	array_push($gmailClique_tabela, array($Row[0].'h', (int)$Row[1], number_format((int)$Row[1],0,',','.')));
	$t_gmailClique += $Row[1];
}
...
foreach($gmailSave as &$Row){
	array_push($gmailSave_tabela, array($Row[0].'h', (int)$Row[1], number_format((int)$Row[1],0,',','.')));
	$t_gmailSave += $Row[1];
}
...
foreach($gmailEnc as &$Row){
	array_push($gmailEnc_tabela, array($Row[0].'h', (int)$Row[1], number_format((int)$Row[1],0,',','.')));
	$t_gmailEnc += $Row[1];
}
...
new google.visualization.BarChart(document.getElementById('chart_gmail_cliques_hour')).draw(data, optionsCliquesGmailHour);
document.getElementById('t_gmailClique_hour').innerHTML='<h5> Total: ".number_format($t_gmailClique,0,',','.')."</h5>';

var data = google.visualization.arrayToDataTable(".json_encode($gmailSave_tabela).");
new google.visualization.BarChart(document.getElementById('chart_gmail_save_hour')).draw(data, optionsGmailSalvosHour);
document.getElementById('t_gmailSave_hour').innerHTML='<h5> Total: ".number_format($t_gmailSave,0,',','.')."</h5>';

var data = google.visualization.arrayToDataTable(".json_encode($gmailEnc_tabela).");
new google.visualization.BarChart(document.getElementById('chart_gmail_enc_hour')).draw(data, optionsEncaminhadosHour);
document.getElementById('t_gmailEnc_hour').innerHTML='<h5> Total: ".number_format($t_gmailEnc,0,',','.')."</h5>';
